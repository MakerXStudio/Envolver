...
const compareData = (variable: EnvVariable | Section, existingVariable: EnvVariable | Section) => {
  if ('value' in variable && 'value' in existingVariable && variable.value !== existingVariable.value) {
    return true
  }

  // TODO: Check the changed comment works
  if ('comment' in variable && 'comment' in existingVariable && variable.comment !== existingVariable.comment) {
    return true
  }
...
const checkExists = (current: EnvVariable | Section, compare: (EnvVariable | Section)[]) =>
  compare.find((value: EnvVariable | Section) => value.name === current.name)
...
export const checkVariables = (current: EnvVariable | Section, compare: (EnvVariable | Section)[]) => {
  ...
  if ('variables' in current && 'variables' in exists) {
  ...
  if (compareData(current as EnvVariable, exists as EnvVariable)) {
    return {
      ...current,
      changed: true,
